...
metabolism <-
  read_xlsx(
    "~/result/nature-protocal-818/IBD/NIHMS1510763-supplement-Dataset_2.xlsx",
    sheet = 1,
    skip = 1
  ) %>% as.data.frame()
#clinical info
...
meta <- meta[-1, ]
write.csv(meta, "metabolism_meta.csv", row.names = F)

#metabolism abundance data

meta.b <- read_excel("NIHMS1510763-supplement-Dataset_1.xlsx",
                    sheet = 1,
...
convert.name <- read.csv("~/result/nature-protocal-818/IBD/name_map.csv")
...
write.csv(metabolism.select, "metabolism_expr.csv")
