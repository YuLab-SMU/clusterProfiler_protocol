...
gs2 <- compareCluster(geneClusters = genelist,fun = "enricher",gson = gson_KO())
...
treeplot(gs2, offset_tiplab = 10) + ggtitle("IBD 2 subtypes gene ORA")
ggsave("IBD_2_subtypes_in_metagenome_ORA(treeplot).pdf", width = 13, height = 5.5)


#IBD-only-compound
cpd.list <-  readRDS("./IBD.mpse.cpd.rds")
...
ggsave("IBD_2_subtypes_in_metabolism_ORA(treeplot).pdf", width = 13,height = 5.5)
