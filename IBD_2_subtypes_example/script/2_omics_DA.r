...
cd.mg <- DA(expr = metagenome, meta = meta.mg,  group = c("Control", "CD"),  
             filter = "CD",
             filter.p = "pvalue")
uc.mg <- DA(expr = metagenome, meta = meta.mg, group = c("Control", "UC"), 
             filter = "UC",
             filter.p = "pvalue")
...
cd.mb <- DA(expr = metabolism, meta = meta.mb, group = c("Control", "CD"), 
            filter = "CD")
uc.mb <- DA(expr = metabolism, meta = meta.mb, group = c("Control", "UC"), 
            filter = "UC")
