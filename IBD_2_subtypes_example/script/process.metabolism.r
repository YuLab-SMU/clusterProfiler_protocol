...
metabolism <-
  read_xlsx(
    "IBD_2_subtypes_example/input_data/NIHMS1510763-supplement-Dataset_2.xlsx",
    sheet = 1,
    skip = 1
  ) %>% as.data.frame()
...
meta <- meta[-1, ]
write.csv(meta, "IBD_2_subtypes_example/input_data/metabolism_meta.csv", row.names = F)

#metabolism abundance data

meta.b <- read_excel("IBD_2_subtypes_example/input_data/NIHMS1510763-supplement-Dataset_1.xlsx",
                    sheet = 1,
...
convert.name <- read.csv("IBD_2_subtypes_example/input_data/name_map.csv")
...
write.csv(metabolism.select, "IBD_2_subtypes_example/input_data/metabolism_expr.csv")
