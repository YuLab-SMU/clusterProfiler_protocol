...
#IBD-only-gene
genelist<- readRDS("../result/IBD.gene.mpse.rds")
...
ggsave("../result/IBD_2_subtypes_gene_ORA.pdf",width =5.5,height=7 )

gs2  <-  pairwise_termsim(gs)
treeplot(gs2, offset_tiplab =9) + ggtitle("IBD 2 subtypes gene ORA")
ggsave("../result/IBD_2_subtypes_in_metagenome_ORA(treeplot).pdf", width = 15, height = 5.5)


#IBD-only-compound
cpd.list <-  readRDS("../result/IBD.cpd.mpse.rds")
...
ggsave("../result/IBD_2_subtypes_in_metabolism_ORA.pdf", width = 5.5, height = 7)

gs2  <-  pairwise_termsim(gs)
treeplot(gs2, offset_tiplab = 10, showCategory = 15) + ggtitle("IBD 2 subtypes compound ORA")
ggsave("../result/IBD_2_subtypes_in_metabolism_ORA(treeplot).pdf", width = 40, height = 5.5)
